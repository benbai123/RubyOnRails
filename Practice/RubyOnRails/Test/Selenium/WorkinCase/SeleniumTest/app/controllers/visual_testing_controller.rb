...
class VisualTestingController < ApplicationController
  ...
  # show cases under test/integration
  def showcases
    fullpath = File.join(Rails.root, "test", "integration")
    @files = get_files(fullpath).reject { |c| c.start_with? '.' }
  end
  # show folder content
  def show
  ...
  def act
    ...
    fullpath = fullpath.split('?').first
    begin
      puts 'delete if exist: '+fullpath
      File.delete(fullpath) if (act.eql?('Delete') && File.exist?(fullpath))
      if act.eql?('Apply')
        ...
        base_rgba = base_path.split('.png').join('.png')+'.rgba'
        diff_path = File.join(case_folder, 'diff_dir', 'diff_'+fname)

        puts 'delete if eists: ' + diff_path
        File.delete(diff_path) if File.exist?(diff_path)
        puts 'delete if eists: ' + base_path
        File.delete(base_path) if File.exist?(base_path)
        puts 'delete if eists: ' + base_rgba
        File.delete(base_rgba) if File.exist?(base_rgba)
        puts 'cp '+fullpath+' '+base_path
        FileUtils.cp fullpath, base_path
        FileUtils.chmod 0777, base_path
      end
      render nothing: true
    rescue => err
      puts err.message
